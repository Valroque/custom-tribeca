...
import Cryptokart = require("./gateways/cryptokart");
...
const liveTradingSetup = () : SimulationClasses => {
    ...
    const getExchange = (): Models.Exchange => {
        ...
        switch (ex) {
            ...
            case "cryptokart": return Models.Exchange.Cryptokart;
    ...
    const getExch = (orderCache: Broker.OrderStateCache): Promise<Interfaces.CombinedGateway> => {
        switch (exchange) {
            ...
            case Models.Exchange.Cryptokart: return Cryptokart.createCryptokart(config, orderCache, timeProvider, pair);
