...
class HitBtcMarketDataGateway implements Interfaces.IMarketDataGateway {
    ...
    constructor(
        ...
        this._tradesClient = io.connect("HitBtcMarketDataUrl");
        ...
        request.get(
            {url: url.resolve(config.GetString("HitBtcPullUrl"), "/api/2/public/orderbook/" + this._symbolProvider.symbol)},
            (err, body, resp) => {
                this.onMarketDataSnapshotFullRefresh(resp, Utils.date());
            });

        request.get(
            {url: url.resolve(config.GetString("HitBtcPullUrl"), "/api/2/public/trades/"+ this._symbolProvider.symbol + "?sort=DESC"),
             qs: {from: 0, by: "trade_id", sort: 'desc', start_index: 0, max_results: 100}},
            (err, body, resp) => {
                JSON.parse(resp).forEach(t => {
                    const price = t.price;
                    const size = t.quantity;
...
class HitBtcOrderEntryGateway implements Interfaces.IOrderEntryGateway {
    ...
    cancelOrder = (cancel : Models.OrderStatusReport) => {
        request(
            ...
            {
                method: 'POST',
                url: `https://test.cryptokart.io:1337/matchengine/order/cancel`,
                headers: {
                    'Authorization': 'Bearer SSi6oSJr3LqaFhBeHzfhqkXJqEEVqYc5TOpYLsd2dHOjpEqTDZvPljxdje5TEXEUlVTZB7VBxwiaf2tgKEVICeZt4YQOmFa4lEde3TuN6ZXZC7QE628YYihCSOLO2pj86DeM8ArUkUazjXEnzmBVwwtzy8GkiDaP2XxBsjp9Zuw7Ui4QRpattqMWDFYHtK80xwPs7o7d76tVBLFIHoY6zC3V0dLaICHNomQ2iKRWCYfRge6cErck715PBulh4yZ',
    ...
    sendOrder = (order : Models.OrderStatusReport) => {
        ...
        request(
            {
                method: 'POST',
                url: HitBtcOrderEntryGateway.getType(order.type) === 'limit' ?
                ...
                formData: {
                    "market_name" : this._symbolProvider.symbol,
                    "side" : HitBtcOrderEntryGateway.getSide(order.side),
                    "amount" : order.quantity * _lotMultiplier,
                    "price" : order.price,
                }
            },
            (err, body, resp) => {
    ...
    constructor(config : Config.IConfigProvider, private _symbolProvider: HitBtcSymbolProvider, private _details: HitBtcBaseGateway) {
        ...
        console.log("==========  ", this._apiKey, this._secret, config.GetString("HitBtcOrderEntryUrl"), "  ================");
...
class HitBtcPositionGateway implements Interfaces.IPositionGateway {
    ...
    // private onTick = () => {
    //     request.get(
    //         this.getAuth("/api/2/trading/balance"),
    //         (err, body, resp) => {
    //             try {
...
export async function createHitBtc(config : Config.IConfigProvider, pair: Models.CurrencyPair) : Promise<Interfaces.CombinedGateway> {
    const symbolsUrl = config.GetString("HitBtcPullUrl") + "/api/2/public/symbol";
    const symbols = await Utils.getJSON<{symbols: HitBtcSymbol[]}>(symbolsUrl);
    const symbolProvider = new HitBtcSymbolProvider(pair);

    for (let s in symbols) {
        if (symbols[s].id === symbolProvider.symbol) 
            return new HitBtc(config, symbolProvider, parseFloat(symbols[s].tickSize), pair);
    }
